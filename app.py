...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000, debug=True)                        proc_info["log_file"].close()
                except Exception:
                    pass
                RUNNING_PROCESSES.pop(server_id, None)
                modified = True
...
def create_server():
    ...
    # আপনার স্ক্রিনশটের মতো server_id (যেমন: testing_1779785743)
    clean_prefix = "".join(c for c in name.lower() if c.isalnum() or c == '_') or "server"
    server_id = f"{clean_prefix}_{int(time.time())}"
